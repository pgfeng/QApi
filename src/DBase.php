...
abstract class DBase
{
     ...
     * @return bool|Data
     */
    final public function getOne($field = '*'): bool|Data
    {
        $this->select($field);
        $this->limit(0, 1);
        $fetch = $this->query();
     ...
     * @return Data|array
     */
    public function findAll($sql = false): Data|array
    {
     ...
     * @return string|array
     */
    public function stripslashes($var): array|string
    {
        if (is_array($var)) {
            foreach ($var as $k => &$v) {
                $this->stripslashes($v);
    ...
    abstract public function _query($sql): array;         //返回值是查询出的数组
     ...
     * @return mixed
     */
    abstract public function _exec($sql): mixed;           //执行SQL
